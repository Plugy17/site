    ...
    <div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-emerald-900">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-32">
          <div className="max-w-3xl">
            ...
            <p className="text-lg text-gray-300 mb-8 max-w-xl">{t('landing.subtitle')}</p>
            <div className="flex flex-col sm:flex-row gap-4">
              <Link to="/courses" className="inline-flex items-center justify-center gap-2 bg-emerald-600 text-white px-8 py-3.5 rounded-xl text-base font-semibold hover:bg-emerald-500 transition-all shadow-lg shadow-emerald-600/25">
                {t('landing.browseCourses')} <ArrowRight className="w-4 h-4" />
              </Link>
              <Link to="/login" className="inline-flex items-center justify-center gap-2 bg-white/10 text-white px-8 py-3.5 rounded-xl text-base font-semibold hover:bg-white/20 transition-all border border-white/20">
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-14">
            <h2 className="text-3xl font-bold text-gray-900 mb-3">{t('landing.everythingTitle')}</h2>
            <p className="text-gray-500 max-w-lg mx-auto">{t('landing.everythingSub')}</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {features.map(f => (
              <div key={f.title} className="group p-6 rounded-2xl border border-gray-100 hover:border-emerald-200 hover:shadow-lg hover:shadow-emerald-50 transition-all">
        ...
        <section className="py-16 bg-gray-50">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-2xl font-bold text-gray-900">{t('nav.courses')}</h2>
              <Link to="/courses" className="text-sm text-emerald-600 hover:underline flex items-center gap-1">{t('landing.exploreAll')} <ArrowRight className="w-3.5 h-3.5" /></Link>
            </div>
            <div className="grid md:grid-cols-3 gap-6">
              {courses.map(course => (
                <Link key={course.id} to={`/courses/${course.id}`} className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg hover:border-emerald-200 transition-all">
                  <div className="h-36 bg-gradient-to-br from-emerald-100 to-teal-100 relative overflow-hidden">
                    {course.thumbnail ? (
                      ...
                      <div className="flex items-center justify-center h-full"><BookOpen className="w-10 h-10 text-emerald-300" /></div>
                    )}
                    <div className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm rounded-full px-2.5 py-1 text-xs font-medium text-emerald-700 capitalize">{course.level}</div>
                  </div>
                  <div className="p-4">
                    <div className="text-xs font-medium text-emerald-600 mb-1">{t(`category.${course.category}`) || course.category}</div>
                    <h3 className="font-semibold text-gray-900 text-sm mb-1 line-clamp-1">{course.title}</h3>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-gray-500 mb-8 max-w-md mx-auto">{t('landing.ctaSub')}</p>
          <Link to="/courses" className="inline-flex items-center gap-2 bg-emerald-600 text-white px-8 py-3.5 rounded-xl text-base font-semibold hover:bg-emerald-500 transition-all shadow-lg shadow-emerald-600/25">
