  ...
  async function handleAuth(provider: 'google' | 'apple') {
    ...
    try {
      ...
      if (err.code === 'auth/popup-blocked') {
        setAuthError(t('auth.popupBlocked') || 'Всплывающее окно заблокировано. Разрешите всплывающие окна для этого сайта.');
      } else if (err.code === 'auth/popup-closed-by-user') {
        setAuthError(t('auth.popupClosed') || 'Окно авторизации закрыто. Попробуйте снова.');
      } else if (err.code === 'auth/unauthorized-domain') {
        setAuthError(t('auth.unauthorizedDomain') || 'Домен не авторизован. Добавьте домен в Firebase Console.');
      } else if (err.code === 'auth/configuration-not-found') {
        setAuthError(t('auth.configNotFound') || 'Ошибка конфигурации Firebase. Проверьте настройки провайдера в Firebase Console.');
      } else if (err.code === 'auth/account-exists-with-different-credential') {
        setAuthError(t('auth.accountExists') || 'Аккаунт уже существует с другим методом входа.');
      } else {
        setAuthError(err.message || 'Ошибка авторизации. Попробуйте снова.');
      }
    }
    setLoading(false);
  }
