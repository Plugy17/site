...
export default function LoginPage() {
  ...
  async function handleGoogleAuth() {
    ...
    try {
      ...
      if (err.code === 'auth/popup-blocked') {
        setAuthError('Всплывающее окно заблокировано. Разрешите всплывающие окна для этого сайта.');
      } else if (err.code === 'auth/popup-closed-by-user') {
        setAuthError('Окно авторизации закрыто. Попробуйте снова.');
      } else if (err.code === 'auth/unauthorized-domain') {
        setAuthError('Домен не авторизован. Добавьте домен в Firebase Console.');
      } else {
  ...
  async function handleAdminLogin(e: React.FormEvent) {
    ...
    if (result.success) {
      ...
      setAdminError(result.error || t('auth.adminError'));
    }
    setLoading(false);
  }
