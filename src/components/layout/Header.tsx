...
import { Menu, X, LogOut, User, Globe, Sun, Moon } from 'lucide-react';
...
export default function Header() {
  ...
  const isInCabinet = !!user;
  const homeLink = isInCabinet ? (profile?.role === 'instructor' ? '/instructor' : '/dashboard') : '/';
        ...
        <div className="flex items-center justify-between h-16">
          <Link to={homeLink} className="flex items-center gap-2">
            <div className="relative w-9 h-9">
              <div className="absolute inset-0 bg-gradient-to-br from-violet-600 to-fuchsia-600 rounded-lg animate-pulse blur-sm opacity-70" />
              <div className="relative w-9 h-9 bg-gradient-to-br from-violet-600 to-fuchsia-600 rounded-lg flex items-center justify-center">
          ...
          <nav className="hidden md:flex items-center gap-6">
            {!isInCabinet && <Link to="/courses" className="text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-violet-600 dark:hover:text-violet-400 transition-colors">{t('nav.courses')}</Link>}
            {user && <Link to={dashboardPath} className="text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-violet-600 dark:hover:text-violet-400 transition-colors">{t('nav.dashboard')}</Link>}
            {profile?.role === 'instructor' && <Link to="/instructor/create" className="text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-violet-600 dark:hover:text-violet-400 transition-colors">{t('nav.createCourse')}</Link>}
              ...
              <div className="flex items-center gap-0.5 bg-gray-100 dark:bg-gray-800 rounded-lg px-1 py-0.5">
                ...
                {langs.map(l => (
                  <button key={l.code} onClick={() => setLang(l.code as any)}
                    className={`px-2 py-1 rounded text-xs font-medium transition-colors ${lang === l.code ? 'bg-violet-600 text-white' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}>
                    {l.label}
                  </button>
                ))}
              </div>
          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              ...
              <Link to="/login" className="bg-violet-600 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-violet-700 transition-colors">
                {t('nav.signIn')}
              </Link>
            )}
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden pb-4 border-t border-gray-100 dark:border-gray-800">
            <div className="flex flex-col gap-2 pt-3">
              {!isInCabinet && <Link to="/courses" onClick={() => setMenuOpen(false)} className="px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg">{t('nav.courses')}</Link>}
              ...
              <div className="flex items-center gap-2 px-3 py-2">
                <div className="flex items-center gap-0.5 bg-gray-100 dark:bg-gray-800 rounded-lg px-1 py-0.5">
                  <Globe className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500 ml-1" />
                  {langs.map(l => (
                    <button key={l.code} onClick={() => setLang(l.code as any)} className={`px-2 py-1 rounded-lg text-xs font-medium transition-colors ${lang === l.code ? 'bg-violet-600 text-white' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'}`}>
                      {l.label}
                    </button>
                  ))}
                </div>
              ...
              {user ? (
                ...
                <Link to="/login" onClick={() => setMenuOpen(false)} className="px-3 py-2 text-sm font-medium text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-900/20 rounded-lg">{t('nav.signIn')}</Link>
              )}
            </div>
          </div>
        )}
